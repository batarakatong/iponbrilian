<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm text-gray-500">Tambah data kenaikan pangkat</p>
    ...
    <div class="py-10">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="card">
                <form method="POST" action="{{ route('promotions.store') }}" class="space-y-6" enctype="multipart/form-data">
                    @csrf
                    @if($users->count())
                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="current_rank" name="current_rank" type="text" class="mt-1 block w-full" :value="old('current_rank')" />
                            <x-input-error class="mt-2" :messages="$errors->get('current_rank')" />
                        </div>
                        <div>
                            <x-input-label for="new_rank" value="Pangkat Baru" />
                            <x-text-input id="new_rank" name="new_rank" type="text" class="mt-1 block w-full" :value="old('new_rank')" required />
