<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm text-gray-500">Tambah data absensi</p>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Catatan Absensi</h2>
            </div>
    ...
    <div class="py-10">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="card">
                <form method="POST" action="{{ route('attendances.store') }}" class="space-y-6" enctype="multipart/form-data">
                    @csrf
                    <div>
                        ...
                        <select id="user_id" name="user_id" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-primary-500 focus:ring-primary-500" required>
                            ...
                            @foreach($users as $optionUser)
                                <option value="{{ $optionUser->id }}" @selected(old('user_id') == $optionUser->id)>{{ $optionUser->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="month" name="month" type="month" class="mt-1 block w-full" :value="old('month')" required />
                            <x-input-error class="mt-2" :messages="$errors->get('month')" />
                        </div>
                        <div>
                            <x-input-label for="present_count" value="Jumlah Hadir" />
                            <x-text-input id="present_count" name="present_count" type="number" min="0" class="mt-1 block w-full" :value="old('present_count', 0)" required />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <x-text-input id="late_count" name="late_count" type="number" min="0" class="mt-1 block w-full" :value="old('late_count', 0)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('late_count')" />
                        </div>
                        <div>
                            <x-input-label for="leave_count" value="Jumlah Izin" />
                            <x-text-input id="leave_count" name="leave_count" type="number" min="0" class="mt-1 block w-full" :value="old('leave_count', 0)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('leave_count')" />
                        </div>
                        <div>
                            <x-input-label for="absent_count" value="Jumlah Absen" />
                            <x-text-input id="absent_count" name="absent_count" type="number" min="0" class="mt-1 block w-full" :value="old('absent_count', 0)" required />
